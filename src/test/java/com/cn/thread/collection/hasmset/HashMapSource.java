package com.cn.thread.collection.hasmset;
import java.io.*;
import java.util.*;

public class HashMapSource<K,V>
    ...
    implements Map<K,V>, Cloneable, Serializable
{
    ...
    // 构造一个带指定初始容量和加载因子的空 HashMapSource。
    ...
    // 构造一个带指定初始容量和默认加载因子 (0.75) 的空 HashMapSource。
    public HashMapSource(int initialCapacity) {
        this(initialCapacity, DEFAULT_LOAD_FACTOR);  
    }  
  
    // 构造一个具有默认初始容量 (16) 和默认加载因子 (0.75) 的空 HashMapSource。
    ...
    // 构造一个映射关系与指定 Map 相同的新 HashMapSource。
    ...
    // 返回此 HashMapSource 实例的浅表副本：并不复制键和值本身
    public Object clone() {  
        HashMapSource<K,V> result = null;
    try {  
    ...
    static class Entry<K,V> implements Map.Entry<K,V> {  
        ...
        // 使用该方法证明该key已经在该map中  
        void recordAccess(HashMapSource<K,V> m) {
        }  
  
        // 该方法记录该key已经被移除了  
        void recordRemoval(HashMapSource<K,V> m) {
        }  
    }  
    ...
    private abstract class HashIterator<E> implements Iterator<E> {
        ...
        public void remove() {  
            ...
            current = null;  
            // 删除掉对应key的元素  
            HashMapSource.this.removeEntryForKey(k);
            // 重置expectedModCount  
            expectedModCount = modCount;  
        }  
     ...
     * D4iterator.remove、Set.remove、removeAll、retainAll 和 clear 操作
    ...
    private final class KeySet extends AbstractSet<K> {  
        ...
        public boolean remove(Object o) {  
            return HashMapSource.this.removeEntryForKey(o) != null;
        }  
        // 定义clear  
        public void clear() {  
            HashMapSource.this.clear();
        }  
    }  
     ...
     * 通过 D4iterator.remove、Collection.remove、removeAll、retainAll 和 clear 操作
     * 可从该映射中移除相应的映射关系。它不支持 add 或 addAll 操作。 
     */  
     ...
     * 通过 D4iterator.remove、Set.remove、removeAll、retainAll 和 clear 操作
     * 可从该映射中移除相应的映射关系。它不支持 add 或 addAll 操作。 
     */  
